...
export default async function EventDetailPage({
  ...
  const { tab = 'qa' } = await searchParams
  const validTabs = ['qa', 'polls', 'discussions', 'info'] as const
  type EventTab = typeof validTabs[number]
  const activeTab: EventTab = (validTabs as readonly string[]).includes(tab) ? tab as EventTab : 'qa'
  ...
  const [questionsRaw, polls, discussionPosts] = await Promise.all([
    activeTab === 'qa' ? getQuestionsForEvent(eventId, userId) : Promise.resolve([]),
    activeTab === 'polls' ? getPollsForEvent(eventId, userId) : Promise.resolve([]),
    activeTab === 'discussions' ? getPostsForEvent(eventId) : Promise.resolve([]),
  ])

  const questions =
    activeTab === 'qa' && questionsRaw.length > 0
      ? await Promise.all(
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <EventTabs
          activeTab={activeTab}
          eventPath={`/p/${slug}/events/${eventId}`}
        />
          ...
          <div className="space-y-4">
            {event.isQaLocked && (
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-700 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
                🔒 Q&A is locked for this event. No new questions or answers can be posted.
              </div>
            )}
        ...
        {activeTab === 'polls' && (
          <div className="space-y-4">
            ...
            {isEventCreator && !event.isPollLocked && (
              <a
                href={`/p/${slug}/events/${eventId}/create-poll`}
            ...
            {polls.length > 0 ? (
              <div className="space-y-4">
                {polls.map((poll) => (
                  <PollCard key={poll._id} poll={poll} userId={userId} eventId={eventId} publicationSlug={slug} />
                ))}
              </div>
            ) : (
              <p className="rounded-xl border border-dashed border-border bg-muted/20 p-8 text-center text-sm text-muted-foreground">
            ...
            <section className="rounded-xl border border-border bg-card p-5">
              ...
              <dl className="space-y-1.5 text-sm">
                ...
                <div className="flex gap-2">
                  ...
                  <dd className="text-foreground font-mono text-xs">{event.timezone}</dd>
                </div>
                {event.creator && (
                  <div className="flex gap-2">
